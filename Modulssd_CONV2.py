...
Updated by Jiaojiao Zhao for ucf101-24 dataset
"""
...
class SSD(nn.Module):
    ...
    def __init__(self, base, extras, head, num_classes):
        ...
        #modulation layer
        #self.sft1_2 = SFTLayer(1,2)
        self.sft2_1 = SFTLayer(2,2,3,1)
    ...
    def forward(self, x):
        ...
        for k, v in enumerate(self.extras):
            fea = F.relu(v(fea), inplace=True)
            if k % 2 == 1:
                sources.append(fea)

        # apply multibox head to source layers
        for (fea, l, c) in zip(sources, self.loc, self.conf):
            loc.append(l(fea).permute(0, 2, 3, 1).contiguous())
            conf.append(c(fea).permute(0, 2, 3, 1).contiguous())
