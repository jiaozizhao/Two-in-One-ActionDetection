...
from Modulssd_CONV2 import build_ssd
...
def test_net(net, save_root, exp_name, input_type, dataset, iteration, num_classes, thresh=0.5 ):
    ...
    frame_save_dir = '/local/ucf24/detections_CONV2/CONV-'+input_type+'-'+args.listid+'-'+str(iteration).zfill(6)+'/'
    ...
    for val_itr in range(len(val_data_loader)):
        ...
        images,flows, targets, img_indexs = next(batch_iterator)
        batch_size = images.size(0)
        height, width = images.size(2), images.size(3)

        if args.cuda:
            images = Variable(images.cuda(), volatile=True)
            flows = Variable(flows.cuda(), volatile=True)
        output = net((images,flows))
...
def main():
   ...
   # means = (104, 117, 123, 120, 120, 120, 120, 120, 120, 120, 120, 120, 120, 120, 120, 120, 120, 120)  # only support voc now
    means = (104, 117, 123, 120, 120, 120)
    exp_name = 'CONV2-Modul-SSD-{}-{}-bs-{}-{}-lr-{:05d}'.format(args.dataset, args.input_type,
